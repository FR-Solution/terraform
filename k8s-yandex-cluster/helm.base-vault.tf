resource "vault_approle_auth_backend_role" "k8s-vault-role" {
  ...
  backend         = module.kubernetes.k8s_global_vars.global_path.base_vault_path_approle
  role_name       = "kubelet-peer-k8s-certmanager"
  token_policies  = ["${module.kubernetes.k8s_global_vars.main_path.base_vault_path}/certificates/vault-cluster-policy"]
  token_ttl               = 60
...
resource "helm_release" "base-vault-node-csr" {
  ...
  values = [
    templatefile("templates/helm/base-vault-node-csr/values.yaml", {
        ...
        vaut_sign_path        : "${module.kubernetes.k8s_global_vars.ssl.intermediate.kubernetes-ca.path}/sign/kubelet-peer-k8s-certmanager"
        vaut_server           : module.kubernetes.k8s_global_vars.vault-config.vault_server
        vaut_approle_path     : module.kubernetes.k8s_global_vars.global_path.base_vault_path_approle
    })
  ]
}
